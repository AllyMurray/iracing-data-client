...
function setupDualLogging(
  samplesDir: string,
  args: string[],
): { logFilePath: string; close: () => Promise<void> } {
...
function addSeasonCarClassPair(
...
function choosePreferredSeasonCarClassPair(
  context: ScrapeContext,
): { seasonId: number; carClassId: number } | undefined {
  const candidates = context.seasonCarClassPairs.filter(
    (pair) => pair.seasonId > 100 && pair.seasonId < 20000 && pair.carClassId > 0,
  );
...
function choosePreferredTeamSeasonCarClassPair(
  context: ScrapeContext,
): { seasonId: number; carClassId: number } | undefined {
  const candidates = context.teamSeasonCarClassPairs.filter(
    (pair) => pair.seasonId > 100 && pair.seasonId < 20000 && pair.carClassId > 0,
  );
  if (candidates.length === 0) return undefined;
...
function topTeamSeasonCarClassPairs(
  context: ScrapeContext,
  limit: number,
): Array<{ seasonId: number; carClassId: number }> {
  const candidates = context.teamSeasonCarClassPairs.filter(
    (pair) => pair.seasonId > 100 && pair.seasonId < 20000 && pair.carClassId > 0,
  );
...
function choosePreferredId(
  values: number[],
  kind: 'season' | 'subsession' | 'carClass' | 'team',
): number | undefined {
...
function applyContextToParams(
  ...
  if (seasonCarClassPair && mapped.season_id !== undefined && mapped.car_class_id !== undefined) {
  ...
  if (
    mapped.subsession_id === undefined &&
    endpoint.params.subsession_id &&
...
async function bootstrapContext(
  ...
  const seeds: Array<{ name: string; url: string; params?: Record<string, any> }> = [
    ...
    {
      name: 'season.spectator_subsessionids',
      url: 'https://members-ng.iracing.com/data/season/spectator_subsessionids',
    },
    {
      name: 'season.spectator_subsessionids_detail',
      url: 'https://members-ng.iracing.com/data/season/spectator_subsessionids_detail',
    },
  ...
  if (preferredPair) {
    console.log(
      `   • preferred season/class pair: ${preferredPair.seasonId}/${preferredPair.carClassId}`,
    );
  }
  const preferredTeamPair = choosePreferredTeamSeasonCarClassPair(context);
  if (preferredTeamPair) {
    console.log(
      `   • preferred TEAM season/class pair: ${preferredTeamPair.seasonId}/${preferredTeamPair.carClassId}`,
    );
  }
}
...
async function fetchWithRetry(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      console.warn(
        `   🔁 Transient error, retrying in ${backoffMs}ms (attempt ${attempt + 1}/${maxAttempts})`,
      );
...
function collectContextFromValue(
  value: unknown,
  context: ScrapeContext,
  ...
  const currentSeasonId =
    typeof record.season_id === 'number' ? record.season_id : inheritedSeasonId;
  ...
  if (typeof record.subsession_id === 'number' && Array.isArray(record.session_results)) {
    for (const session of record.session_results) {
      ...
      const simsessionNumber =
        typeof (session as any).simsession_number === 'number'
          ? (session as any).simsession_number
          : 0;
      ...
      for (const row of rows) {
        ...
        const teamId =
          typeof (row as any).team_id === 'number' && (row as any).team_id > 0
            ? (row as any).team_id
            : undefined;
        context.lapDataCandidates.push({
          subsessionId: record.subsession_id,
          simsessionNumber,
          custId,
          teamId,
        });
  ...
  if (
    ...
    typeof record.max_team_drivers === 'number' &&
    record.max_team_drivers > 1
  ) {
    if (typeof record.car_class_id === 'number') {
      context.teamSeasonCarClassPairs.push({
        seasonId: currentSeasonId,
        carClassId: record.car_class_id,
      });
    }
    if (Array.isArray(record.car_class_ids)) {
      for (const carClassId of record.car_class_ids) {
...
function loadContextFromExistingSamples(samplesDir: string): ScrapeContext {
  ...
  if (process.env.IRACING_SUBSESSION_ID)
    addUnique(context.subsessionIds, Number(process.env.IRACING_SUBSESSION_ID));
  if (process.env.IRACING_SEASON_ID)
    addUnique(context.seasonIds, Number(process.env.IRACING_SEASON_ID));
  if (process.env.IRACING_CAR_CLASS_ID)
    addUnique(context.carClassIds, Number(process.env.IRACING_CAR_CLASS_ID));
...
function generateSampleParams(
  ...
  const now = new Date();
  const recentIso = new Date(now.getTime() - (includeOptional ? 14 : 7) * 24 * 60 * 60 * 1000)
    .toISOString()
    .replace(/\.\d{3}Z$/, 'Z');
  const recentIsoEnd = new Date(now.getTime() - (includeOptional ? 2 : 1) * 24 * 60 * 60 * 1000)
    .toISOString()
    .replace(/\.\d{3}Z$/, 'Z');
...
function generateParameterVariations(
  endpoint: FlatEndpoint,
  custId: number,
  context: ScrapeContext,
): Record<string, any>[] {
  ...
  const seasonId = choosePreferredId(context.seasonIds, 'season');
  const subsessionId = choosePreferredId(context.subsessionIds, 'subsession');
  const subsessionCandidates = [...new Set(context.subsessionIds.filter((id) => id > 100000))]
    .slice(-4)
    .reverse();
  const simsessionCandidates = [
    ...new Set(context.simsessionNumbers.filter((id) => typeof id === 'number' && id <= 0)),
  ].slice(0, 3);
  if (!simsessionCandidates.includes(0)) simsessionCandidates.unshift(0);
  const custCandidates = [...new Set([custId, ...context.custIds.filter((id) => id > 0)])].slice(
    0,
    8,
  );
  const teamId = choosePreferredId(context.teamIds, 'team');
  ...
  switch (endpoint.fullName) {
    case 'results.get':
      ...
      if (subsessionId !== undefined)
        variations.push({ subsession_id: subsessionId, include_licenses: false });
      break;
    case 'results.event_log':
      for (const candidateSubsessionId of subsessionCandidates) {
        for (const simsessionNumber of simsessionCandidates) {
          variations.push({
            subsession_id: candidateSubsessionId,
            simsession_number: simsessionNumber,
          });
        }
      }
      if (subsessionId !== undefined)
        variations.push({ subsession_id: subsessionId, simsession_number: 0 });
      break;
    ...
    case 'stats.season_team_standings':
      for (const candidate of teamSeasonCarClassCandidates) {
        variations.push({
          ...
          car_class_id: candidate.carClassId,
      ...
      if (teamSeasonCarClassPair) {
        variations.push({
          season_id: teamSeasonCarClassPair.seasonId,
          car_class_id: teamSeasonCarClassPair.carClassId,
          race_week_num: raceWeekNum,
        });
      }
      if (seasonCarClassPair) {
        variations.push({
          season_id: seasonCarClassPair.seasonId,
          car_class_id: seasonCarClassPair.carClassId,
          race_week_num: raceWeekNum,
        });
  ...
  if (endpoint.fullName === 'results.search_hosted') {
    normalized = normalized.filter(
      (v) =>
        (v.start_range_begin || v.finish_range_begin) &&
        (v.cust_id || v.team_id || v.host_cust_id || v.session_name),
    );
  }

  if (endpoint.fullName === 'results.search_series') {
    normalized = normalized.filter(
      (v) => v.start_range_begin || v.finish_range_begin || (v.season_year && v.season_quarter),
    );
  }
...
async function scrapeApiSamples() {
  ...
  if (!clientId || !clientSecret || !email || !password) {
    throw new Error(
      'IRACING_CLIENT_ID, IRACING_CLIENT_SECRET, IRACING_USERNAME, and IRACING_PASSWORD must be set',
    );
  }
  ...
  if (MISSING_ONLY) {
    ...
    endpoints = endpoints.filter(
      (endpoint) => !hasAnySampleForEndpoint(SAMPLES_DIR, endpoint.fullName),
    );
  ...
  for (let i = 0; i < endpoints.length; i++) {
    ...
    try {
      if (
        ['results.get', 'results.event_log', 'results.lap_chart_data', 'results.lap_data'].includes(
          endpoint.fullName,
        ) &&
        scrapeContext.subsessionIds.length === 0
      ) {
        console.log(
          '   ⚠️  Missing subsession IDs. Set IRACING_SUBSESSION_ID or scrape season.spectator_subsessionids first.',
        );
      }
      if (endpoint.fullName === 'team.get' && scrapeContext.teamIds.length === 0) {
        console.log(
          '   ⚠️  Missing team IDs. Set IRACING_TEAM_ID or ensure team.membership returns at least one team.',
        );
      }
      if (
        [
        ...
        console.log(
          '   ⚠️  Missing season IDs. Set IRACING_SEASON_ID or ensure season/series samples are present.',
        );
      }
      if (
        [
        ...
        console.log(
          '   ⚠️  Missing car class IDs. Set IRACING_CAR_CLASS_ID or ensure series.seasons samples include car_class_ids.',
        );
      }
      ...
      for (let varIndex = 0; varIndex < paramVariations.length; varIndex++) {
        ...
        if (
          endpoint.fullName.includes('league') &&
          endpoint.fullName !== 'league.get_points_systems'
        ) {
        ...
        if (
          endpoint.fullName.includes('time_attack') &&
          endpoint.params.ta_comp_season_id?.required
        ) {
