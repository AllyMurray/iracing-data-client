...
export function generateZodSchemaFromSample(sample: any, schemaName: string): string {
  ...
  function mergeArrayOfObjects(items: any[], depth: number = 0): string {
    ...
    for (const [fieldKey, allItems] of Object.entries(nestedArrays)) {
      if (
        allItems.length > 0 &&
        allItems.every((item) => item && typeof item === 'object' && !Array.isArray(item))
      ) {
  ...
  function generateZodObjectSchema(obj: any, depth: number = 0): string {
    ...
    if (
      ...
      keys.includes('inactive') &&
      keys.includes('active') &&
      keys.includes('pitroad')
    ) {
      return `z.object({
...
${objIndent}})`;
    ...
    if (
      keys.includes('car_dirpath') &&
      keys.includes('car_id') &&
      keys.includes('rain_enabled') &&
...
${objIndent}})`;
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (
        key.startsWith('__nullable_') ||
        key.startsWith('__optional_') ||
        key === '__fieldValues'
      ) {
        continue;
      }
    ...
    if (sample.length > 0) {
      ...
      if (sample.every((item) => item && typeof item === 'object' && !Array.isArray(item))) {
        ...
        for (const [fieldKey, objects] of Object.entries(nestedObjects)) {
          ...
          if (
            nonNullObjects.length > 0 &&
            !mergedObject[fieldKey]?.__dictionary_unknown &&
            !mergedObject[fieldKey]?.__recordSchema
          ) {
            // Recursively merge non-null nested objects
            ...
            const objectSchema = mergedNestedSchema
              .replace(/^const \w+ = z\.array\(/, '')
              .replace(/\);$/, '');
  ...
  if (typeof sample === 'object' && sample !== null) {
    ...
    if (isDictionary) {
      ...
      const hasPrimitives = allValues.some(
