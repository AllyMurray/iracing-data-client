...
function generateParamValidator(name: string, params: Record<string, ParamDef>): string[] {
  ...
  for (const [paramName, paramDef] of Object.entries(params)) {
    ...
    if (camelParamName !== paramName) {
      lines.push(
        `  ${camelParamName}: ${getZodTypeForParam(paramDef)},${comment} // maps to: ${paramName}`,
      );
    } else {
...
export function generateSectionTest(sectionName: string, endpoints: Flat[]): string {
  ...
  lines.push('');
  lines.push(
    `    ${toCamelCase(sectionName)}Service = new ${toPascal(sectionName)}Service(client);`,
  );
  lines.push('  });');
  lines.push('');
  ...
  for (const endpoint of endpoints) {
    ...
    if (endpoint.samplePath) {
      lines.push(
        `    it("should fetch and validate ${sectionName} ${methodName} data", async () => {`,
      );
      ...
      if (shouldPassParams) {
        ...
        lines.push(
          `      const result = await ${toCamelCase(sectionName)}Service.${methodName}(testParams);`,
        );
      } else {
        lines.push(
          `      const result = await ${toCamelCase(sectionName)}Service.${methodName}();`,
        );
      }
...
export function generateSectionService(sectionName: string, endpoints: Flat[]): string {
  ...
  for (const endpoint of endpoints) {
    ...
    const schemaName =
      endpoint.responseType && endpoint.samplePath
        ? endpoint.responseType.replace('Response', '')
        : null;
    ...
    if (hasParams) {
      lines.push(
        `  async ${methodName}(params${paramsAreRequired ? '' : '?'}: ${paramsType}): Promise<${responseType}> {`,
      );
    } else {
    ...
    if (hasParams) {
      if (sectionName === 'stats' && semanticPairValidationMethods.has(methodName)) {
        lines.push(
          `    await this.client.ensureSeasonCarClassPair('stats.${endpoint.name.split('.').pop()}', params.seasonId, params.carClassId);`,
        );
      }
      if (schemaName) {
        lines.push(
          `    return this.client.get<${responseType}>("${endpoint.url}", { params, paramsValidator: ${paramsValidator}, schema: ${schemaName} });`,
        );
      } else {
        lines.push(
          `    return this.client.get<${responseType}>("${endpoint.url}", { params, paramsValidator: ${paramsValidator} });`,
        );
      }
    } else {
      if (schemaName) {
        lines.push(
          `    return this.client.get<${responseType}>("${endpoint.url}", { schema: ${schemaName} });`,
        );
      } else {
