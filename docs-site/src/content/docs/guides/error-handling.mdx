...
<CardGrid>
  ...
  <Card title="⏱️ Rate Limiting" icon="moon">
    **Status: 429**
  ...
  <Card title="🚫 Not Found" icon="x">
    **Status: 404**
    
    Requested resource doesn't exist.
  </Card>
  <Card title="🧪 Invalid Params" icon="warning">
    **Status: 400**
  ...
  <TabItem label="Authentication">
    ...
    try {
      ...
      if (error instanceof IRacingError && error.isUnauthorized) {
        ...
        if (!process.env.IRACING_CLIENT_ID || !process.env.IRACING_CLIENT_SECRET) {
          throw new Error('Missing iRacing credentials in environment');
        }

        // Credentials might be invalid
        console.error('Please check your iRacing credentials');
        process.exit(1);
      }
    }
    ```
...
- Review the [Client API](/iracing-data-client/api/client/) for error class details
- See [Authentication Guide](/iracing-data-client/getting-started/authentication/) for auth error handling
- Check [Rate Limiting Guide](/iracing-data-client/guides/rate-limiting/) for handling rate limits
