...
The iRacing API enforces rate limits to ensure fair usage and system stability. The SDK detects rate limit responses and throws an `IRacingError` with `isRateLimited = true`. By default it does not retry failed requests. You can opt in to automatic retry with backoff using the `retry` option, and you can opt in to local request throttling with a pluggable `stores.rateLimit` implementation.

<Aside type="note">
  Request queue helpers are still planned. The current built-in integrations are `retry` for transient failures and `stores.rateLimit` for local request throttling.
</Aside>

## Pluggable Rate Limit Store

<Aside type="tip">
  Install `@http-client-toolkit/store-memory`, `@http-client-toolkit/store-sqlite`, `@http-client-toolkit/store-dynamodb`, or another compatible toolkit store package separately if you want a ready-made rate limit store.
</Aside>
...
// Requests now wait locally when the configured store is exhausted
const data = await iracing.member.info();
```

The SDK scopes toolkit rate-limit buckets by request URL origin. For normal iRacing Data API calls, the resource key is `https://members-ng.iracing.com`.

The `stores` option is completely optional. When omitted, the SDK behaves exactly as before.

Check the [`http-client-toolkit` repository](https://github.com/AllyMurray/http-client-toolkit) for available store packages, or implement the `RateLimitStore` interface yourself.
...
### Strategy 1: Built-In Retry with Backoff

Enable built-in retry for transient failures, including 429 and 5xx responses:
