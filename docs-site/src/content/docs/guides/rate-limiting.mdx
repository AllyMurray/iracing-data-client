...
<Aside type="note">
  The SDK supports pluggable rate limiting via the `stores.rateLimit` option (see below). This guide also covers additional strategies you can implement yourself.
</Aside>
...
### Strategy 2: Custom Rate Limiter

For fine-grained control beyond what the pluggable `RateLimitStore` provides, you can implement your own concurrency limiter:
...
For a token bucket approach, you can use an `AdaptiveRateLimitStore` via the `stores` option, or implement your own:
