...
<Tabs>
  ...
  <TabItem label="Authorization Code">
    ...
    interface AuthorizationCodeAuth {
      ...
      tokens: {
        ...
        refreshToken?: string;
        expiresAt?: number;
      };                             // Required: obtained via code exchange
      onTokenRefresh?: OnTokenRefresh; // Callback on token refresh
    }
    ```
  </TabItem>
  ...
  <TabItem label="validateParams">
    **Type:** `boolean` (optional, default `true`)
    ...
    Enables or disables runtime parameter validation.

    This controls generated Zod schema validation only.
  ...
  <TabItem label="validateSemanticParams">
    **Type:** `boolean` (optional, default `true`)
    ...
    Enables endpoint-specific semantic preflight checks where implemented.

    Example: validating `seasonId`/`carClassId` pairs for standings endpoints before making the API request.

    These checks may perform additional metadata requests on first use.
...
<Card title="Response Caching *(coming soon)*">
  Response caching is on the roadmap. See our [roadmap](/iracing-data-client/roadmap/) for details.
</Card>
...
<Aside type="caution">
  The iRacing API has rate limits. The client throws an `IRacingError` with `isRateLimited = true` when limits are exceeded. See the [Rate Limiting guide](/iracing-data-client/guides/rate-limiting/) for handling strategies.
</Aside>
