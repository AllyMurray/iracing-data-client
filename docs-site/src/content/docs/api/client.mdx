...
interface IRacingClientOptions {
  ...
  // Optional pluggable stores for caching, deduplication, and rate limiting
...
<Tabs>
  ...
  <TabItem label="stores">
    **Type:** `HttpClientStores` (optional)
    ...
    Pluggable stores for caching, deduplication, and rate limiting, powered by `@http-client-toolkit/core`. When omitted, the SDK operates without external stores (same behaviour as previous versions).
    ...
    See the [Caching Guide](/guides/caching/) and [Rate Limiting Guide](/guides/rate-limiting/) for detailed examples.
  </TabItem>
</Tabs>
...
// IRacingError is a class — use a value import for instanceof checks
...
class IRacingWrapper {
  private sdk: IRacingDataClient;
  
  constructor(options: IRacingClientOptions) {
    this.sdk = new IRacingDataClient(options);
  }
