...
<Tabs>
  ...
  <TabItem label="stores">
    **Type:** `HttpClientStores` (optional)
    ...
    - **`cache`** — A `CacheStore` for caching resolved GET results. For presigned S3 responses, TTL is automatically derived from the URL expiry.
...
When a `CacheStore` is provided via the `stores` option, the client caches resolved GET results through `@http-client-toolkit/core`:

- Cached results are returned immediately without making a new request when the entry is fresh
- For iRacing presigned S3 responses, cache TTL is calculated from the S3 link's `expires` field
- Direct responses use normal HTTP cache headers, or the toolkit fallback TTL when no cache headers are present
