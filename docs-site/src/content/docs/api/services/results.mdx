...
<Tabs>
  ...
  <TabItem label="Example">
    ...
    console.log(`Matching rows: ${results.data.chunkInfo.rows}`);
    ...
    console.log(seriesResults.data.chunkInfo.chunkFileNames);
    ```
  </TabItem>
</Tabs>
...
<Tabs>
  ...
  <TabItem label="Example">
    ...
    console.log(`Matching rows: ${leagueResults.data.chunkInfo.rows}`);
    ...
    console.log(hostedResults.data.chunkInfo.chunkFileNames);
    ```
  </TabItem>
</Tabs>
...
async function analyzeRace(subsessionId: number) {
  const [results, laps, eventLog] = await Promise.all([
    ...
    iracing.results.lapData({ subsessionId, simsessionNumber: 0 }),
    iracing.results.eventLog({ subsessionId, simsessionNumber: 0 }),
  ]);
