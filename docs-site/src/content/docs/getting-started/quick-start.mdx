...
<Aside type="tip">
  This guide assumes you have Node.js 22 or newer installed and an active iRacing subscription. CI tests Node.js 22 and 24 LTS.
</Aside>
...
<Aside type="note">
  You can optionally pass pluggable `stores` for caching, rate limiting, and request deduplication via the `stores` option on `IRacingDataClient`. See the [Caching Guide](/iracing-data-client/guides/caching/) and [Rate Limiting Guide](/iracing-data-client/guides/rate-limiting/) for details.
</Aside>
