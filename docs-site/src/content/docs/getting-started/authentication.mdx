...
const iracing1 = new IRacingDataClient({
  email: process.env.IRACING_EMAIL,
  password: process.env.IRACING_PASSWORD
});
...
const iracing2 = new IRacingDataClient({
  headers: {
    'Cookie': authCookies
  }
});
...
<CardGrid>
  ...
  <Card title="🚫 Client-Side Security" icon="warning">
    Never use the Data Client with credentials in browser JavaScript. Always authenticate on the backend.
  </Card>
...
Mock the Data Client for unit tests:

```typescript
// __mocks__/iracing-data-client.ts
export const IRacingDataClient = jest.fn().mockImplementation(() => ({
  member: {
...
class IRacingMultiAccount {
  private accounts: Map<string, IRacingDataClient> = new Map();
  
  addAccount(name: string, email: string, password: string) {
    this.accounts.set(name, new IRacingDataClient({ email, password }));
  }
  
  getAccount(name: string): IRacingDataClient {
