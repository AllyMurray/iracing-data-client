...
const cli = resolve(dirname(manifestPath), manifest.bin.dotenvx);
const fixture = require('./fixtures/dotenvx-v1.json');
const temporary = mkdtempSync(join(tmpdir(), 'iracing-env-'));
...
const environment = Object.fromEntries(Object.entries(process.env)
  .filter(([key]) => !key.startsWith('DOTENV') && !key.startsWith('IRACING')));
...
function run(code, privateKey) {
  return spawnSync(process.execPath, [cli, 'run', ...isolated, '--quiet', '--strict',
...
try {
  ...
  writeFileSync(join(temporary, '.env'), fixture.encryptedEnv);
  expectSuccess(run(assertion, fixture.privateKey));
  assert.equal(run('process.exit(17)', fixture.privateKey).status, 17,
  ...
  writeFileSync(join(temporary, '.env'), `DOTENVX_SMOKE_VALUE=${fixture.expected}\n`);
  const encrypted = command(['encrypt', ...isolated]);
  assert.equal(encrypted.status, 0, encrypted.error?.message || encrypted.stderr);
  const keys = readFileSync(join(temporary, '.env.keys'), 'utf8');
  ...
  rmSync(join(temporary, '.env.keys'));
